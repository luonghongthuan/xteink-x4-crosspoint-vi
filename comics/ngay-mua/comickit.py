...
# ===== Mo rong: nhan vat vui nhon + hieu ung cho truyen thieu nhi =====

@_extend(Page)
def dog(self, cx, cy, s=1.0, mood="happy"):
    """Chu cho tron, tai cup, luoi le."""
    d = self.d; r = int(30 * s)
    d.ellipse([cx - r, cy - r, cx + r, cy + r], fill=WHITE, outline=BLACK, width=3)
    # tai cup hai ben
    d.ellipse([cx - r - 6, cy - r + 4, cx - r + 12, cy + 20], fill=GRAY1, outline=BLACK, width=3)
    d.ellipse([cx + r - 12, cy - r + 4, cx + r + 6, cy + 20], fill=GRAY1, outline=BLACK, width=3)
    for ex in (cx - 11, cx + 11):
        d.ellipse([ex - 4, cy - 6, ex + 4, cy + 2], fill=BLACK)
    d.ellipse([cx - 6, cy + 6, cx + 6, cy + 16], fill=BLACK)  # mui
    if mood == "happy":
        d.arc([cx - 12, cy + 8, cx + 12, cy + 24], 20, 160, fill=BLACK, width=2)
        d.polygon([(cx + 4, cy + 18), (cx + 12, cy + 30), (cx + 2, cy + 26)], fill=GRAY1, outline=BLACK)  # luoi
    else:
        d.line([cx - 8, cy + 16, cx + 8, cy + 16], fill=BLACK, width=2)


@_extend(Page)
def rabbit(self, cx, cy, s=1.0, mood="happy"):
    """Tho tai dai."""
    d = self.d; r = int(26 * s)
    for ex in (-10, 10):  # tai
        d.ellipse([cx + ex - 8, cy - r - 40, cx + ex + 8, cy - r + 6], fill=WHITE, outline=BLACK, width=3)
        d.ellipse([cx + ex - 3, cy - r - 32, cx + ex + 3, cy - r], fill=GRAY1)
    d.ellipse([cx - r, cy - r, cx + r, cy + r], fill=WHITE, outline=BLACK, width=3)
    for ex in (cx - 9, cx + 9):
        d.ellipse([ex - 3, cy - 3, ex + 3, cy + 5], fill=BLACK)
    d.ellipse([cx - 4, cy + 6, cx + 4, cy + 12], fill=BLACK)
    if mood == "happy":
        d.arc([cx - 8, cy + 8, cx + 8, cy + 20], 20, 160, fill=BLACK, width=2)
    elif mood == "wow":
        d.ellipse([cx - 5, cy + 8, cx + 5, cy + 18], outline=BLACK, width=2)


@_extend(Page)
def bird(self, cx, cy, s=1.0):
    d = self.d; r = int(16 * s)
    d.ellipse([cx - r, cy - r, cx + r, cy + r], fill=GRAY1, outline=BLACK, width=2)
    d.polygon([(cx + r - 2, cy), (cx + r + 10, cy - 4), (cx + r + 10, cy + 4)], fill=BLACK)  # mo
    d.ellipse([cx + 2, cy - 6, cx + 8, cy], fill=BLACK)  # mat
    d.arc([cx - r - 8, cy - 8, cx + 2, cy + 12], 30, 150, fill=BLACK, width=2)  # canh


@_extend(Page)
def sun(self, cx, cy, s=1.0):
    import math
    d = self.d; r = int(26 * s)
    for a in range(0, 360, 30):
        x1 = cx + int((r + 4) * math.cos(math.radians(a)))
        y1 = cy + int((r + 4) * math.sin(math.radians(a)))
        x2 = cx + int((r + 16) * math.cos(math.radians(a)))
        y2 = cy + int((r + 16) * math.sin(math.radians(a)))
        d.line([x1, y1, x2, y2], fill=BLACK, width=2)
    d.ellipse([cx - r, cy - r, cx + r, cy + r], fill=GRAY1, outline=BLACK, width=3)
    d.arc([cx - 12, cy - 2, cx + 12, cy + 14], 20, 160, fill=BLACK, width=2)
    for ex in (cx - 9, cx + 9):
        d.ellipse([ex - 2, cy - 6, ex + 2, cy - 2], fill=BLACK)


@_extend(Page)
def tree(self, cx, yfloor, s=1.0):
    d = self.d; w = int(18 * s); h = int(70 * s); cr = int(46 * s)
    d.rectangle([cx - w // 2, yfloor - h, cx + w // 2, yfloor], fill=GRAY2, outline=BLACK, width=2)
    d.ellipse([cx - cr, yfloor - h - cr, cx + cr, yfloor - h + cr // 2], fill=GRAY1, outline=BLACK, width=3)


@_extend(Page)
def emote(self, cx, cy, kind):
    """Hieu ung cam xuc noi tren dau: sao, tim, giot mo hoi, dau hoi, tia."""
    d = self.d
    if kind == "star":
        import math
        pts = []
        for i in range(10):
            rr = 14 if i % 2 == 0 else 6
            a = math.radians(i * 36 - 90)
            pts.append((cx + rr * math.cos(a), cy + rr * math.sin(a)))
        d.polygon(pts, fill=BLACK)
    elif kind == "heart":
        d.pieslice([cx - 12, cy - 8, cx, cy + 4], 0, 360, fill=BLACK)
        d.pieslice([cx, cy - 8, cx + 12, cy + 4], 0, 360, fill=BLACK)
        d.polygon([(cx - 11, cy - 1), (cx + 11, cy - 1), (cx, cy + 14)], fill=BLACK)
    elif kind == "sweat":
        d.ellipse([cx - 5, cy - 2, cx + 5, cy + 12], fill=WHITE, outline=BLACK, width=2)
        d.polygon([(cx, cy - 10), (cx - 5, cy), (cx + 5, cy)], fill=WHITE, outline=BLACK)
    elif kind == "q":
        self.d.text((cx - 8, cy - 14), "?", fill=BLACK, font=font(30))
    elif kind == "excl":
        self.d.text((cx - 5, cy - 14), "!", fill=BLACK, font=font(30))
    elif kind == "burst":
        import math
        for a in range(0, 360, 45):
            x = cx + int(20 * math.cos(math.radians(a))); y = cy + int(20 * math.sin(math.radians(a)))
            d.line([cx, cy, x, y], fill=BLACK, width=2)


@_extend(Page)
def bigtitle(self, text, y=40, sz=44):
    """Tieu de CHU TO, vien dam de noi bat tren bia."""
    f = font(sz)
    w = self.d.textbbox((0, 0), text, font=f)[2]
    x = (W - w) // 2
    for dx, dy in [(-2, 0), (2, 0), (0, -2), (0, 2)]:
        self.d.text((x + dx, y + dy), text, fill=BLACK, font=f)
    self.d.text((x, y), text, fill=BLACK, font=f)
